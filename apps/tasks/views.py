...
class TaskListView(LoginRequiredMixin, PaginationMixin, ListView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            # exec_result['detail'] = json.dumps(rbt.get_playbook_result(),indent=4)
            exec_result = json.dumps(rbt.get_playbook_result(),indent=4)
            exec_result1 = rbt.get_playbook_result()
            task.detail_result = exec_result
            task.status = 'Y'
            task.save()
        except:
        ...
        task_result.unreachable = len(simple['unreachable'])
        task_result.skipped = len(simple['skipped'])
        task_result.ok = len(simple['ok'])
        task_result.changed = len(simple['changed'])
        task_result.failures = len(simple['failed'])
        task_result.save()
