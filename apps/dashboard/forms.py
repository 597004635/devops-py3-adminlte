...
class UserProfileForm(forms.ModelForm):
    ...
    def __init__(self,*args,**kwargs):
        ...
        self.fields['name_cn'].label = '账 号'
        self.fields['name_cn'].error_messages = {'required': '请输入中文名字'}
        ...
        self.fields['phone'].error_messages={'required': '请输入电话号码'}

    def clean_password(self):
        password = self.cleaned_data.get('password')
        if len(password) < 6  :
            raise forms.ValidationError('密码必须大于6位/密码为空')
        return password
